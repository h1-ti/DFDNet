...
class DFLPNG(object):
    ...
    def get_face_type(self):
        return self.dfl_dict['face_type']
    ...
    def get_xseg_mask(self):
        ...
        if xseg_mask is not None:
            # xseg_mask = np.clip(np.array(xseg_mask, np.float32) / 255.0, 0.0, 1.0)
            # if len(xseg_mask.shape) == 2:
            #     xseg_mask = xseg_mask[..., np.newaxis]
            return xseg_mask
        return None

    def _get_fanseg_mask(self):
        _fanseg_mask = self.dfl_dict.get ('fanseg_mask', None)
        _fanseg_mask = np.array (_fanseg_mask, np.uint8)[..., np.newaxis]
        return _fanseg_mask
